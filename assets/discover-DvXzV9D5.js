...
function wordFitsInShapeAdaptive(cx, cy, halfW, halfH, shapeCenterX, shapeCenterY, shapeRadius, shape) {
  ...
  if (sdf < -rMax && shape === "circle") return { fits: true, zone: "deep" }
...
function findPlacement(grid, halfW, halfH, shapeCenterX, shapeCenterY, shapeRadius, shape, canvasW, canvasH, allowRotation) {
  ...
  for (var iter = 0; iter < MAX_SPIRAL_ITERATIONS; iter++) {
    ...
    // SDF raymarching: jump over dead zones
    ...
    var sx = tx, sy = ty
    // Adaptive shape check with bounding sphere early exit
