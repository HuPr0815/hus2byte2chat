...
/* ================================================================== */
/*  Inlined: shapes.ts v8 — analytic SDF, adaptive, push-to-edge      */
/*  v8: heart Y-flip, tangent field, feature anchors, curvature        */
/* ================================================================== */
...
function computeLayout(words, config) {
  ...
  if (shape !== "circle") {
    var anchorCount = Math.max(2, Math.min(6, Math.ceil(sorted.length * 0.06)))
  ...
  for (var i = 0; i < sorted.length; i++) {
    ...
    if (shape !== "circle" && skeletonAnchorIdx < skeletonAnchors.length && i < Math.ceil(sorted.length * 0.10)) {
      for (var level = startLevel; level < 4; level++) {
        ...
        for (var oi = 0; oi < offsets.length; oi++) {
          ...
          if (placement) {
            ...
            if (cappedFs < fontSize) {
              var sizeRatio = cappedFs / fontSize
              actualW = w * sizeRatio
              actualH = h * sizeRatio
            }
    ...
    // Phase 2: Normal placement (isocontour or spiral) with v8 enhancements
    if (!bestResult) {
      for (var level = startLevel; level < 4; level++) {
        if (level > startLevel) {
          fontSize = Math.max(minFontSize, mapFontSize(word.weight, maxWeight, minFontSize, maxFontSize) * SIZE_RATIOS[level])
    ...
    placed.push({
      ...
      fontSize: bestResult.fs, width: bestResult.actualW, height: bestResult.actualH, rotated: bestResult.rotated
    })
    grid.markOccupied(bestResult.x, bestResult.y,
      bestResult.rotated ? bestResult.actualH / 2 : bestResult.actualW / 2,
      bestResult.rotated ? bestResult.actualW / 2 : bestResult.actualH / 2)
  }
