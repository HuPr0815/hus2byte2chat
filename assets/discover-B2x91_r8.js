import{t as e}from"./book-open-OfpCZycm.js";import{t}from"./chevron-down-BC4e88Mz.js";import{t as n}from"./rotate-ccw-BJSilb9J.js";import{n as r,t as i}from"./zoom-out-CPD5AU_Z.js";import{D as a,F as o,I as s,K as c,L as l,P as u,R as d,U as f,c as p,l as m,s as h}from"./index-Ca5ETVYC.js";import{t as g}from"./head-seo-BHlxNDQT.js";import{t as _}from"./stagger-animation-5BN1nadJ.js";var v=o(`hash`,[[`line`,{x1:`4`,x2:`20`,y1:`9`,y2:`9`,key:`4lhtct`}],[`line`,{x1:`4`,x2:`20`,y1:`15`,y2:`15`,key:`vyu0kd`}],[`line`,{x1:`10`,x2:`8`,y1:`3`,y2:`21`,key:`1ggp8o`}],[`line`,{x1:`16`,x2:`14`,y1:`3`,y2:`21`,key:`weycgp`}]]),y=c(f(),1),b=class{gridScale;gridWidth;gridHeight;canvasWidth;canvasHeight;data;stride;sat;satStride;totalOccupied=0;mipScale;mipData=null;mipWidth=0;mipHeight=0;mipStride=0;constructor(e,t,n=3,r=4){this.gridScale=n,this.canvasWidth=e,this.canvasHeight=t,this.mipScale=r,this.gridWidth=Math.ceil(e/n),this.gridHeight=Math.ceil(t/n),this.stride=Math.ceil(this.gridWidth/32),this.data=new Uint32Array(this.stride*this.gridHeight),this.satStride=this.gridWidth+1,this.sat=new Uint32Array(this.satStride*(this.gridHeight+1)),this.gridWidth>=r&&this.gridHeight>=r&&(this.mipWidth=Math.ceil(this.gridWidth/r),this.mipHeight=Math.ceil(this.gridHeight/r),this.mipStride=Math.ceil(this.mipWidth/32),this.mipData=new Uint32Array(this.mipStride*this.mipHeight))}clear(){this.data.fill(0),this.sat.fill(0),this.totalOccupied=0,this.mipData?.fill(0)}toGrid(e,t){return[Math.floor(e/this.gridScale),Math.floor(t/this.gridScale)]}markOccupied(e,t,n,r){let[i,a]=this.toGrid(e-n,t-r),[o,s]=this.toGrid(e+n,t+r),c=Math.max(0,i),l=Math.max(0,a),u=Math.min(this.gridWidth-1,o),d=Math.min(this.gridHeight-1,s);new Uint32Array(this.stride);for(let e=l;e<=d;e++){let t=e*this.stride,n=Math.floor(c/32),r=Math.floor(u/32);if(n===r)this.data[t+n]|=(1<<u-c+1)-1<<(c&31);else{this.data[t+n]|=~((1<<(c&31))-1);for(let e=n+1;e<r;e++)this.data[t+e]=4294967295;this.data[t+r]|=(1<<(u&31)+1)-1}}if(this.rebuildSat(l,d),this.mipData){let e=Math.floor(c/this.mipScale),t=Math.floor(u/this.mipScale),n=Math.floor(l/this.mipScale),r=Math.floor(d/this.mipScale);for(let i=n;i<=r;i++){let n=i*this.mipStride;for(let r=e;r<=t;r++){let e=n+Math.floor(r/32);this.mipData[e]|=1<<(r&31)}}}this.totalOccupied+=(u-c+1)*(d-l+1)}rebuildSat(e,t){let n=Math.max(0,e-1);for(let e=n;e<=t;e++){let t=e*this.stride,n=(e+1)*this.satStride,r=e*this.satStride,i=0;for(let e=0;e<this.gridWidth;e++){let a=this.data[t+Math.floor(e/32)]>>(e&31)&1;i+=a,this.sat[n+e+1]=this.sat[r+e+1]+i}}}satQuery(e,t,n,r){let i=Math.max(0,e),a=Math.max(0,t),o=Math.min(this.gridWidth-1,n),s=Math.min(this.gridHeight-1,r);if(i>o||a>s)return 0;let c=this.sat[a*this.satStride+i],l=this.sat[a*this.satStride+(o+1)],u=this.sat[(s+1)*this.satStride+i];return this.sat[(s+1)*this.satStride+(o+1)]-l-u+c}densityAt(e,t,n){let[r,i]=this.toGrid(e,t),a=Math.ceil(n/this.gridScale),o=r-a,s=i-a,c=r+a,l=i+a,u=this.satQuery(o,s,c,l),d=(Math.min(this.gridWidth-1,c)-Math.max(0,o)+1)*(Math.min(this.gridHeight-1,l)-Math.max(0,s)+1);return d>0?u/d:0}checkCollision(e,t,n,r){let[i,a]=this.toGrid(e-n,t-r),[o,s]=this.toGrid(e+n,t+r);if(i<0||o>=this.gridWidth||a<0||s>=this.gridHeight)return!0;let c=Math.floor(i/32),l=Math.floor(o/32);for(let e=a;e<=s;e++){let t=e*this.stride;if(c===l){let e=(1<<o-i+1)-1<<(i&31);if(this.data[t+c]&e)return!0}else{let e=~((1<<(i&31))-1);if(this.data[t+c]&e)return!0;for(let e=c+1;e<l;e++)if(this.data[t+e])return!0;let n=(1<<(o&31)+1)-1;if(this.data[t+l]&n)return!0}}return!1}mipBlockOccupied(e,t){if(!this.mipData)return!1;let n=Math.floor(e/this.mipScale),r=Math.floor(t/this.mipScale);if(n<0||n>=this.mipWidth||r<0||r>=this.mipHeight)return!0;let i=r*this.mipStride+Math.floor(n/32);return(this.mipData[i]&1<<(n&31))!=0}get freeCells(){return this.gridWidth*this.gridHeight-this.totalOccupied}get totalCells(){return this.gridWidth*this.gridHeight}estimateMaxFreeArea(){if(!this.mipData)return this.freeCells*this.gridScale*this.gridScale;let e=0;for(let t=0;t<this.mipData.length;t++)e+=32-x(this.mipData[t]);return e*this.mipScale*this.mipScale*this.gridScale*this.gridScale}};function x(e){return e-=e>>>1&1431655765,e=(e&858993459)+(e>>>2&858993459),e=e+(e>>>4)&252645135,e*16843009>>>24}function S(e,t,n){switch(n){case`circle`:return Math.sqrt(e*e+t*t)-1;case`heart`:{let n=e*1.2,r=-t*1.1,i=n*n+r*r-1,a=i*i*i-n*n*r*r*r,o=i*i,s=6*n*o-2*n*r*r*r,c=6*r*o-3*n*n*r*r,l=Math.sqrt(s*s+c*c);return l<1e-8?Math.sqrt(n*n+r*r)-.9:a/(l*1.15)}case`star`:{let n=Math.sqrt(e*e+t*t);if(n<.001)return-1;let r=Math.atan2(t,e),i=Math.PI*2/5,a=(r%i+i)%i,o=Math.PI/5;return n-(a<o?.42+a/o*.58:1-.58*((a-o)/o))}case`diamond`:return Math.abs(e)+Math.abs(t)-1;case`cloud`:{let n=[[0,0,.48],[-.38,.12,.4],[.38,.12,.4],[-.22,-.28,.44],[.22,-.28,.44],[0,.32,.34]],r=1/0;for(let[i,a,o]of n){let n=Math.sqrt((e-i)**2+(t-a)**2)-o;n<r&&(r=n)}return r}case`hexagon`:{let n=Math.sqrt(e*e+t*t);if(n<.001)return-1;let r=(Math.atan2(t,e)%(Math.PI/3)+Math.PI/3)%(Math.PI/3);return n-Math.cos(Math.PI/6)/Math.cos(r-Math.PI/6)}}}function C(e,t,n){switch(n){case`circle`:return e*e+t*t<=1;case`heart`:{let n=e*1.2,r=-t*1.1,i=n*n+r*r-1;return i*i*i-n*n*r*r*r<=.04}case`star`:{let n=Math.sqrt(e*e+t*t);if(n<.001)return!0;let r=Math.atan2(t,e),i=Math.PI*2/5,a=(r%i+i)%i,o=Math.PI/5;return n<=(a<o?.42+a/o*.58:1-.58*((a-o)/o))}case`diamond`:return Math.abs(e)+Math.abs(t)<=1;case`cloud`:for(let[n,r,i]of[[0,0,.48],[-.38,.12,.4],[.38,.12,.4],[-.22,-.28,.44],[.22,-.28,.44],[0,.32,.34]])if((e-n)**2+(t-r)**2<=i*i)return!0;return!1;case`hexagon`:{let n=Math.sqrt(e*e+t*t);if(n<.001)return!0;let r=(Math.atan2(t,e)%(Math.PI/3)+Math.PI/3)%(Math.PI/3);return n<=Math.cos(Math.PI/6)/Math.cos(r-Math.PI/6)}}}function w(e,t,n){return S(e,t,n)}function T(e,t){let n=e*1.2,r=-t*1.1,i=n*n+r*r-1,a=i*i,o=6*n*a-2*n*r*r*r,s=6*r*a-3*n*n*r*r;if(Math.sqrt(o*o+s*s)<1e-8){let e=Math.sqrt(n*n+r*r)||1;return[-n/e,-r/e]}let c=o*1.2,l=-s*1.1,u=Math.sqrt(c*c+l*l);return[c/u,l/u]}function E(e,t){let n=Math.sqrt(e*e+t*t);if(n<.001)return[-1,0];let r=Math.atan2(t,e),i=Math.PI*2/5,a=(r%i+i)%i,o=Math.PI/5,s=a<o?.58/o:-.58/o,c=e/n-s*(-t/(n*n)),l=t/n-s*(e/(n*n)),u=Math.sqrt(c*c+l*l);return u<1e-8?[-e/n,-t/n]:[c/u,l/u]}function D(e,t,n){if(n===`heart`)return T(e,t);if(n===`star`)return E(e,t);let r=S(e,t,n),i=Math.max(.001,Math.abs(r)*.01),a=S(e+i,t,n)-r,o=S(e,t+i,n)-r,s=Math.sqrt(a*a+o*o);if(s<1e-8){let n=Math.sqrt(e*e+t*t)||1;return[-e/n,-t/n]}return[a/s,o/s]}function O(e,t,n){let r=.005,i=S(e,t,n),a=S(e+r,t,n),o=S(e-r,t,n),s=S(e,t+r,n),c=S(e,t-r,n),l=(a-2*i+o)/(r*r),u=(s-2*i+c)/(r*r);return Math.abs(l)+Math.abs(u)}function k(e,t,n){if(n!==`heart`)return 1;let r=O(e,t,n),i=S(e,t,n);return r>80&&i>-.15?.4:r>50&&i>-.1?.5:r>30&&i>-.05?.65:r>15?.8:1}function A(e,t,n){let[r,i]=D(e,t,n);return Math.atan2(-i,r)}function j(e,t,n,r,i,a,o,s){let c=(e-i)/o,l=(t-a)/o,u=n/o,d=r/o;if(s===`circle`){let e=S(c,l,s),t=Math.sqrt(u*u+d*d);if(e>t)return{fits:!1,zone:`outside`};if(e<-t)return{fits:!0,zone:`deep`};let n=[[c,l],[c-u,l-d],[c+u,l-d],[c-u,l+d],[c+u,l+d]];for(let[e,t]of n)if(!C(e,t,s))return{fits:!1,zone:`edge`};return{fits:!0,zone:`edge`}}let f=s===`heart`||s===`star`?4:3,p=[[c,l],[c-u,l-d],[c+u,l-d],[c-u,l+d],[c+u,l+d]],m=0;for(let[e,t]of p)C(e,t,s)&&m++;return m>=f?{fits:!0,zone:m>=5?`deep`:`edge`}:{fits:!1,zone:`edge`}}function M(e,t,n,r,i,a,o,s,c=5){let l=e,u=t;for(let e=0;e<c;e++){let e=(l-i)/o,t=(u-a)/o,c=S(e,t,s);if(c<=0){if(j(l,u,n,r,i,a,o,s).fits)return[l,u];let[c,d]=D(e,t,s);l-=c*n*.3,u-=d*r*.3;continue}let[d,f]=D(e,t,s),p=Math.abs(c)*o;l-=d*p,u-=f*p}return[l,u]}function N(e,t,n,r,i,a,o,s,c,l,u){let d=S((e-n)/i,(t-r)/i,a);if(d<-.3)return c;let f=Math.max(l,u)/i;if(d>f)return s;if(a===`heart`){let e=(d+f*.3)/(f*1.3);return Math.max(s,Math.min(c,o-Math.max(0,Math.min(1,e))*(o-s)*.5))}let p=(d+f)/(2*f);return Math.max(s,Math.min(c,o-Math.max(0,Math.min(1,p))*(o-s)))}function P(e){switch(e){case`heart`:return[[-.35,-.28],[.35,-.28],[0,.55],[-.18,-.08],[.18,-.08],[0,-.12],[-.5,.15],[.5,.15]];case`star`:{let e=[];for(let t=0;t<5;t++){let n=t/5*Math.PI*2-Math.PI/2,r=n+Math.PI/5;e.push([Math.cos(n)*.9,Math.sin(n)*.9]),e.push([Math.cos(r)*.35,Math.sin(r)*.35]);let i=n+Math.PI/10;e.push([Math.cos(i)*.6,Math.sin(i)*.6])}return e.push([0,0]),e}case`diamond`:return[[0,-.85],[0,.85],[-.85,0],[.85,0],[-.4,-.4],[.4,-.4],[-.4,.4],[.4,.4]];case`cloud`:return[[0,-.35],[-.38,-.12],[.38,-.12],[-.22,.28],[.22,.28],[0,.05]];case`hexagon`:{let e=[];for(let t=0;t<6;t++){let n=t/6*Math.PI*2-Math.PI/6;e.push([Math.cos(n)*.9,Math.sin(n)*.9])}return e.push([0,0]),e}case`circle`:return[[0,0]]}}function ee(e,t){if(e===`circle`)return[[0,0]];let n=P(e),r=[];for(let i of n){if(r.length>=t)break;C(i[0],i[1],e)&&r.push([i[0],i[1]])}if(r.length<t){let n=[];for(let t=0;t<60;t++)for(let r=0;r<60;r++){let i=t/59*2-1,a=r/59*2-1;C(i,a,e)&&n.push({x:i,y:a,sdf:S(i,a,e)})}n.sort((e,t)=>e.sdf-t.sdf);for(let e of n){if(r.length>=t)break;let n=!1;for(let[t,i]of r)if((e.x-t)**2+(e.y-i)**2<.03){n=!0;break}n||r.push([e.x,e.y])}}return r}var F=new Map;function I(e,t,n){return`${e}:${t.toFixed(4)}:${n.toFixed(4)}`}function*L(e,t,n=.06,r=.02){if(e===`circle`){let e=1+t;if(e<=0)return;let r=Math.ceil(2*Math.PI/n),i=Array.from({length:r},(e,t)=>t);for(let e=i.length-1;e>0;e--){let t=Math.floor(Math.random()*(e+1));[i[e],i[t]]=[i[t],i[e]]}for(let t of i){let n=t/r*2*Math.PI;yield[e*Math.cos(n),e*Math.sin(n)]}return}let i=I(e,t,n),a=F.get(i);if(a){let e=Array.from({length:a.length},(e,t)=>t);for(let t=e.length-1;t>0;t--){let n=Math.floor(Math.random()*(t+1));[e[t],e[n]]=[e[n],e[t]]}for(let t of e)yield[a[t][0],a[t][1]];return}let o=Math.ceil(2*Math.PI/n),s=n*.4,c=[],l=e===`heart`?12:8,u=e===`heart`?5e-4:.002;for(let n=0;n<o;n++){let i=n/o*2*Math.PI+(Math.random()-.5)*s,a=.35+Math.abs(t)*.6,d=a*Math.cos(i),f=a*Math.sin(i),p=!1;for(let n=0;n<l;n++){let n=S(d,f,e)-t;if(Math.abs(n)<u){p=!0;break}let[r,i]=D(d,f,e),a=r*r+i*i;if(a<1e-10)break;let o=n/a;d-=r*o,f-=i*o;let s=Math.sqrt(d*d+f*f);s>1.5&&(d*=1.5/s,f*=1.5/s)}if(p&&e===`heart`){let[t,n]=D(d,f,e);if(t*t+n*n<r*r)continue}S(d,f,e)<.02&&c.push([d,f])}if(c.length<10&&n>.02){yield*L(e,t,n*.5,r);return}F.set(i,c);let d=Array.from({length:c.length},(e,t)=>t);for(let e=d.length-1;e>0;e--){let t=Math.floor(Math.random()*(e+1));[d[e],d[t]]=[d[t],d[e]]}for(let e of d)yield[c[e][0],c[e][1]]}function R(e,t,n,r,i){let a=Math.max(e,t)/n;return r===`heart`&&i!==void 0?i>-.06?a>.1?.06:.03:i>-.2?a>.1?.08:.04:a>.15?.12:.06:r===`star`||r===`cloud`?a>.15?.18:a>.08?.12:.08:a>.15?.12:a>.08?.08:.05}function te(e,t,n,r){let i=S(e,t,n);return Math.abs(i)*r*.7}function ne(e,t,n,r,i,a){let o=te(e,t,i,a);return n>o||r>o}var re={circle:{icon:`●`,label:`圆形`},heart:{icon:`♥`,label:`心形`},star:{icon:`★`,label:`星形`},diamond:{icon:`◆`,label:`菱形`},cloud:{icon:`☁`,label:`云形`},hexagon:{icon:`⬡`,label:`六边形`}},ie=[`circle`,`heart`,`star`,`diamond`,`cloud`,`hexagon`],ae={width:800,height:600,shape:`circle`,minFontSize:11,maxFontSize:42,padding:3,gridScale:3,rotationSteps:0,fontFamily:`sans-serif`,fontWeight:`bold`},z=.12,B=.4,V=2e4,H=[1,.75,.5625,.421875],U=/[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]/;function W(e){let t=0;for(let n of e)U.test(n)&&t++;return t>e.length*.5}function G(e,t,n,r){return t<=0?n:n+(r-n)*Math.sqrt(e/t)}function K(e,t,n,r,i,a,o,s,c,l){let u=Math.random()*Math.PI*2,d=0,f=null,p=Math.sqrt(t*t+n*n)*.5,m=e.gridScale;for(let h=0;h<V;h++){let h=r+d*Math.cos(u),g=i+d*Math.sin(u);if(h-t<0||h+t>=s||g-n<0||g+n>=c){u+=z,d+=B*z;continue}let _=w((h-r)/a,(g-i)/a,o);if(_>.15){d+=Math.abs(_)*a*.8+p,u+=z;continue}if(e.mipBlockOccupied(Math.floor(h/m),Math.floor(g/m))){d+=2*B*z,u+=z;continue}let[v,y]=[h,g];if(!j(v,y,t,n,r,i,a,o).fits){let[e,s]=M(v,y,t,n,r,i,a,o,4);if(!j(e,s,t,n,r,i,a,o).fits){u+=z,d+=B*z;continue}v=e,y=s}if(!e.checkCollision(v,y,t,n)){let r=e.densityAt(v,y,Math.max(t,n)*1.5);if((f===null||r<f.score)&&(f={x:v,y,score:r,rotated:!1},r<.08))break}if(l){let[l,u]=[n,t];if(v-l>=0&&v+l<s&&y-u>=0&&y+u<c&&j(v,y,l,u,r,i,a,o).fits&&!e.checkCollision(v,y,l,u)){let t=e.densityAt(v,y,Math.max(l,u)*1.5);if((f===null||t<f.score)&&(f={x:v,y,score:t,rotated:!0},t<.08))break}}u+=z,d+=B*z}return f}var q=[-.4,-.5,-.62,-.32,-.26,-.2,-.16,-.12,-.09,-.06,-.04,-.02],J=[-.02,-.04,-.06,-.09,-.12,-.16,-.2,-.26,-.32,-.4,-.5,-.62],oe=[-.45,-.55,-.25,-.18,-.1,-.06,-.04,-.02],se=[-.02,-.04,-.06,-.1,-.18,-.25,-.45,-.55];function ce(e,t,n,r,i,a,o,s,c,l,u){let d=e.gridScale,f=o===`star`||o===`cloud`,p=u<.25?f?oe:q:f?se:J,m=null,h=0;for(let u=0;u<p.length;u++){let f=p[u],g=R(t,n,a,o,f);for(let[p,_]of L(o,f,g)){let f=r+p*a,g=i+_*a;if(f-t<0||f+t>=s||g-n<0||g+n>=c||e.mipBlockOccupied(Math.floor(f/d),Math.floor(g/d))||o===`heart`&&ne(p,_,t,n,o,a))continue;let[v,y]=[f,g];if(!j(v,y,t,n,r,i,a,o).fits){let[e,s]=M(v,y,t,n,r,i,a,o,3);if(!j(e,s,t,n,r,i,a,o).fits)continue;v=e,y=s}let b=(v-r)/a,x=(y-i)/a,S=w(b,x,o),C=Math.max(t,n)/a,T=S>0&&S<=C*1.5;if(!e.checkCollision(v,y,t,n)){let r=e.densityAt(v,y,Math.max(t,n)*1.5);if((m===null||r<m.score)&&(m={x:v,y,score:r,rotated:!1},h=u,r<.03))return m}if(T&&l){let l=A(b,x,o);for(let d of[l,l+Math.PI/2]){let l=Math.abs(Math.cos(d))<.3,[f,p]=l?[n,t]:[t,n];if(v-f>=0&&v+f<s&&y-p>=0&&y+p<c&&j(v,y,f,p,r,i,a,o).fits&&!e.checkCollision(v,y,f,p)){let t=e.densityAt(v,y,Math.max(f,p)*1.5);if((m===null||t<m.score)&&(m={x:v,y,score:t,rotated:l},h=u,t<.03))return m}}}if(l){let[l,d]=[n,t];if(v-l>=0&&v+l<s&&y-d>=0&&y+d<c&&j(v,y,l,d,r,i,a,o).fits&&!e.checkCollision(v,y,l,d)){let t=e.densityAt(v,y,Math.max(l,d)*1.5);if((m===null||t<m.score)&&(m={x:v,y,score:t,rotated:!0},h=u,t<.03))return m}}}if(m&&m.score<.08&&h<=u)return m}return m||(u>.7?K(e,t,n,r,i,a,o,s,c,l):null)}function le(e,t,n,r,i,a,o,s,c,l,u,d){let f=a+t*s,p=o+n*s;return f-r<0||f+r>=l||p-i<0||p+i>=u||!j(f,p,r,i,a,o,s,c).fits||e.checkCollision(f,p,r,i)?null:{x:f,y:p,rotated:!1}}function ue(e,t,n,r,i,a,o,s,c,l,u){return o===`circle`?K(e,t,n,r,i,a,o,s,c,l):ce(e,t,n,r,i,a,o,s,c,l,u)}function de(e,t){let n=t.estimateMaxFreeArea();return e<=n*.3?0:e<=n*.8?1:e<=n*1.5?2:3}function fe(e,t={}){let n={...ae,...t};if(e.length===0)return{words:[],placedCount:0,totalCount:0,fillRate:0};let{width:r,height:i,shape:a,minFontSize:o,maxFontSize:s,padding:c,gridScale:l,rotationSteps:u,fontFamily:d,fontWeight:f}=n,p=[...e].sort((e,t)=>t.weight-e.weight),m=Math.max(...p.map(e=>e.weight),1),h=r/2,g=i/2,_=Math.min(r,i)/2*.85,v=new b(r,i,l),y=null,x=()=>(y||=document.createElement(`canvas`).getContext(`2d`),y),S=[],C=u>=1,w=[],T=0;a!==`circle`&&(w=ee(a,Math.max(2,Math.min(8,Math.ceil(p.length*.08)))));for(let e=0;e<p.length;e++){let t=p[e],n=G(t.weight,m,o,s),l,u;if(t.width!==void 0&&t.height!==void 0)l=t.width+c*2,u=t.height+c*2;else{let e=x();if(!e)break;e.font=`${f} ${n}px ${d}`,l=e.measureText(t.text).width+c*2,u=(W(t.text)?n:n*1.2)+c*2}let y=de(l*u,v),b=null;if(a!==`circle`&&T<w.length&&e<Math.ceil(p.length*.15)){for(let e=y;e<4;e++){if(e>y){let r=H[e];n=Math.max(o,G(t.weight,m,o,s)*r);let i=x();if(!i)break;i.font=`${f} ${n}px ${d}`,l=i.measureText(t.text).width+c*2,u=(W(t.text)?n:n*1.2)+c*2}let p=l/2,S=u/2,[E,D]=w[T],O=[[0,0],[.08,0],[-.08,0],[0,.08],[0,-.08]],k=!1;for(let[e,t]of O){let c=le(v,E+e,D+t,p,S,h,g,_,a,r,i,C);if(c){let e=N(c.x,c.y,h,g,_,a,s,o,n,p,S);b={x:c.x,y:c.y,fs:e,rotated:c.rotated},k=!0;break}}if(k)break}T++}if(!b)for(let S=y;S<4;S++){if(S>y){let e=H[S];n=Math.max(o,G(t.weight,m,o,s)*e);let r=x();if(!r)break;r.font=`${f} ${n}px ${d}`,l=r.measureText(t.text).width+c*2,u=(W(t.text)?n:n*1.2)+c*2}let w=l/2,T=u/2,E=ue(v,w,T,h,g,_,a,r,i,C,e/p.length);if(E){let e=n,t=k((E.x-h)/_,(E.y-g)/_,a);t<1&&(e=Math.max(o,e*t));let r=N(E.x,E.y,h,g,_,a,s,o,e,w,T);b={x:E.x,y:E.y,fs:r,rotated:E.rotated};break}}if(!b)for(let e=y;e<4;e++){let n=H[e]*.6,l=Math.max(o,G(t.weight,m,o,s)*n),u=x();if(!u)break;u.font=`${f} ${l}px ${d}`;let p=(u.measureText(t.text).width+c*2)/2,y=((W(t.text)?l:l*1.2)+c*2)/2,S=a===`star`||a===`cloud`?se:J;for(let e of S){let t=R(p,y,_,a,e);for(let[n,o]of L(a,e,t,.01)){let e=h+n*_,t=g+o*_;if(!(e-p<0||e+p>=r||t-y<0||t+y>=i)&&j(e,t,p,y,h,g,_,a).fits){if(v.checkCollision(e,t,p,y)){let[r,i]=D(n,o,a),s=e-r*2,c=t-i*2;if(j(s,c,p,y,h,g,_,a).fits&&!v.checkCollision(s,c,p,y)){b={x:s,y:c,fs:l,rotated:!1};break}continue}b={x:e,y:t,fs:l,rotated:!1};break}}if(b)break}if(b)break}if(!b)continue;let E={id:t.id,text:t.text,x:b.x,y:b.y,fontSize:b.fs,width:l,height:u,rotated:b.rotated};S.push(E),v.markOccupied(b.x,b.y,b.rotated?u/2:l/2,b.rotated?l/2:u/2)}let E=0;for(let e of S)E+=e.width*e.height;let O=r*i>0?E/(r*i):0;return{words:S,placedCount:S.length,totalCount:e.length,fillRate:O}}var pe=8,me=3e4;function he(e,t={},n={}){let{timeout:r=me}=n,i=_e();return new Promise((n,a)=>{let o=new Blob([i],{type:`application/javascript`}),s=URL.createObjectURL(o),c=new Worker(s,{type:`classic`}),l=setTimeout(()=>{c.terminate(),URL.revokeObjectURL(s),a(Error(`WordCloud worker timed out`))},r);c.onmessage=e=>{if(clearTimeout(l),c.terminate(),URL.revokeObjectURL(s),e.data.type===`SUCCESS`){let{buffer:t,texts:r,placedCount:i,totalCount:a,fillRate:o}=e.data;n({words:ge(t,r),placedCount:i,totalCount:a,fillRate:o})}else e.data.type===`ERROR`?a(Error(e.data.error)):a(Error(`Unknown worker response`))},c.onerror=e=>{clearTimeout(l),c.terminate(),URL.revokeObjectURL(s),a(Error(e.message))},c.postMessage({type:`compute`,words:e,config:t})})}function ge(e,t){let n=e.length/pe,r=[];for(let i=0;i<n;i++){let n=i*pe,a=e[n+7];r.push({id:e[n],x:e[n+1],y:e[n+2],fontSize:e[n+3],width:e[n+4],height:e[n+5],rotated:e[n+6]!==0,text:t[a]||``})}return r}function _e(){return`
/* ================================================================== */
...
function wordFitsInShapeAdaptive(cx, cy, halfW, halfH, shapeCenterX, shapeCenterY, shapeRadius, shape) {
  ...
  // Non-circle: permissive check — 4/5 for heart/star, 3/5 for others
  var threshold = (shape === "heart" || shape === "star") ? 4 : 3
...
var isocontourCache = new Map()
function cacheKey(shape, targetSdf, angleStep) {
  return shape + ":" + targetSdf.toFixed(4) + ":" + angleStep.toFixed(4)
}
...
function walkIsocontour(shape, targetSdf, angleStep, minGradientMag) {
  ...
  var key = cacheKey(shape, targetSdf, angleStep)
  var cached = isocontourCache.get(key)
  if (cached) {
    var indices = []
    for (var i = 0; i < cached.length; i++) { indices.push(i) }
    ...
    return {
      next: function() {
        if (idx < cached.length) {
          var ci = indices[idx]; idx++
          var p = cached[ci]
          return { value: [p[0], p[1]], done: false }
        }
        return { done: true }
      },
  ...
  var points = []
  var maxIter = (shape === "heart") ? 12 : 8
  var tol = (shape === "heart") ? 0.0005 : 0.002
  ...
  isocontourCache.set(key, points)
...
function isocontourAngleStep(halfW, halfH, shapeRadius, shape, targetSdf) {
  ...
  if (shape === "star" || shape === "cloud") {
    var wordSize = Math.max(halfW, halfH) / shapeRadius
    if (wordSize > 0.15) return 0.18
    if (wordSize > 0.08) return 0.12
    return 0.08
  }
  var wordSize = Math.max(halfW, halfH) / shapeRadius
...
var ISO_LEVELS_HEART_DEEP = [-0.40, -0.50, -0.62, -0.32, -0.26, -0.20, -0.16, -0.12, -0.09, -0.06, -0.04, -0.02]
var ISO_LEVELS_HEART_BOUNDARY = [-0.02, -0.04, -0.06, -0.09, -0.12, -0.16, -0.20, -0.26, -0.32, -0.40, -0.50, -0.62]
var ISO_LEVELS_FAST_DEEP = [-0.40, -0.50, -0.62, -0.32, -0.20, -0.12, -0.06, -0.02]
var ISO_LEVELS_FAST_BOUNDARY = [-0.02, -0.06, -0.12, -0.20, -0.32, -0.40, -0.50, -0.62]
...
function findPlacementIsocontour(grid, halfW, halfH, shapeCenterX, shapeCenterY, shapeRadius, shape, canvasW, canvasH, allowRotation, wordRankPct) {
  var gridScale = grid.gridScale
  var isFast = shape === "star" || shape === "cloud"
  var deepLevels = isFast ? ISO_LEVELS_FAST_DEEP : ISO_LEVELS_HEART_DEEP
  var boundaryLevels = isFast ? ISO_LEVELS_FAST_BOUNDARY : ISO_LEVELS_HEART_BOUNDARY
  var isoLevels = wordRankPct < 0.25 ? deepLevels : boundaryLevels
  ...
  for (var i = 0; i < sorted.length; i++) {
    ...
    if (!bestResult) {
      for (var level = startLevel; level < 4; level++) {
        ...
        var mw = w / 2, mh = h / 2
        var found = false
        var microLevels = (shape === "star" || shape === "cloud") ? ISO_LEVELS_FAST_BOUNDARY : ISO_LEVELS_HEART_BOUNDARY
        for (var sdfLevel of microLevels) {
...
`}function ve(e,t={}){let n=performance.now(),r=fe(e,t),i=performance.now()-n;return i>50&&console.debug(`[wordcloud] computeSync took ${i.toFixed(1)}ms for ${e.length} words`),r}function ye(e,t={}){return e.length>100?he(e,t):ve(e,t)}var Y=s(),X=`"Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif`,be=1e3,xe=2400,Se=.25,Ce=4,Z=.15;function Q(e,t,n){let r=t/100,i=n/100,a=r*Math.min(i,1-i),o=t=>{let n=(t+e/30)%12;return i-a*Math.max(-1,Math.min(n-3,9-n,1))},s=Math.max(0,Math.min(255,Math.round(o(0)*255))),c=Math.max(0,Math.min(255,Math.round(o(8)*255))),l=Math.max(0,Math.min(255,Math.round(o(4)*255)));return`#${s.toString(16).padStart(2,`0`)}${c.toString(16).padStart(2,`0`)}${l.toString(16).padStart(2,`0`)}`}function we(e,t,n){let r=document.createElement(`canvas`).getContext(`2d`);if(!r)return{inputs:[],colors:new Map,totalArea:0};let i=[...e].sort((e,t)=>t.articleCount-e.articleCount),a=new Map,o=0;return{inputs:i.map((e,i)=>{let s=e.articleCount/t,c=11+31*Math.sqrt(s);r.font=`bold ${c}px ${X}`;let l=r.measureText(e.name),u=i*137.508%360,d=n?68:42;a.set(e.id,[u,70,d]);let f=l.width+6,p=c+6;return o+=f*p,{id:e.id,text:e.name,weight:e.articleCount,width:l.width,height:c}}),colors:a,totalArea:o}}function Te(e,t,n,r,i,a,o){let s=window.devicePixelRatio||1;e.width=t*s,e.height=n*s,e.style.width=t+`px`,e.style.height=n+`px`;let c=e.getContext(`2d`);if(c){c.scale(s,s),c.clearRect(0,0,t,n);for(let e=0;e<r.length;e++){let t=r[e],n=e===a,s=i.get(t.id);if(!s)continue;let[l,u,d]=s,f=n?Math.min(d+18,90):d;c.save(),c.font=`bold ${t.fontSize}px ${X}`,c.textAlign=`center`,c.textBaseline=`middle`;let p=c.createLinearGradient(t.x-t.width/2,t.y-t.height/2,t.x-t.width/2,t.y+t.height/2);p.addColorStop(0,Q(l,u,Math.min(f+8,95))),p.addColorStop(.5,Q(l,u,f)),p.addColorStop(1,Q(l,u,Math.max(f-6,5))),c.fillStyle=p,c.shadowColor=o?`rgba(0,0,0,0.5)`:`rgba(0,0,0,0.18)`,c.shadowBlur=n?8:3,c.shadowOffsetX=1,c.shadowOffsetY=1,c.fillText(t.text,t.x,t.y),c.restore()}}}function Ee({tags:e,shape:t=`circle`,onShapeChange:a}){let o=(0,y.useRef)(null),s=(0,y.useRef)(null),c=d(),[l,u]=(0,y.useState)(1),[f,p]=(0,y.useState)(0),[m,h]=(0,y.useState)(0),[g,_]=(0,y.useState)(!1),v=(0,y.useRef)({x:0,y:0,px:0,py:0}),[b,x]=(0,y.useState)(null),[S,C]=(0,y.useState)({x:0,y:0}),[w,T]=(0,y.useState)(!1),[E,D]=(0,y.useState)(()=>window.matchMedia(`(prefers-color-scheme: dark)`).matches);(0,y.useEffect)(()=>{let e=window.matchMedia(`(prefers-color-scheme: dark)`),t=e=>D(e.matches);return e.addEventListener(`change`,t),()=>e.removeEventListener(`change`,t)},[]);let O=(0,y.useMemo)(()=>Math.max(...e.map(e=>e.articleCount),1),[e]),{inputs:k,colors:A,totalArea:j}=(0,y.useMemo)(()=>we(e,O,E),[e,O,E]),M=(0,y.useMemo)(()=>{let e=j/.6,t=Math.ceil(Math.sqrt(e)*1.15);return Math.max(be,Math.min(xe,t))},[j]),N=M,P=M,[ee,F]=(0,y.useState)({w:800,h:500});(0,y.useEffect)(()=>{let e=o.current;if(!e)return;let t=()=>{let t=e.clientWidth;F({w:t,h:Math.min(t*.78,600)})};t();let n=new ResizeObserver(()=>t());return n.observe(e),()=>n.disconnect()},[]);let[I,L]=(0,y.useState)([]);(0,y.useEffect)(()=>{if(k.length===0||N===0){L([]);return}let e=!1,n=ye(k,{width:N,height:P,shape:t,minFontSize:11,maxFontSize:42,padding:3,gridScale:3});return n instanceof Promise?n.then(t=>{e||(L(t.words),R(t.words,N,P))}):(L(n.words),R(n.words,N,P)),()=>{e=!0}},[k,t,N,P]);let R=(0,y.useCallback)((e,t,n)=>{if(e.length===0)return;let r=o.current;if(!r)return;let i=1/0,a=1/0,s=-1/0,c=-1/0;for(let t of e)i=Math.min(i,t.x-t.width/2),a=Math.min(a,t.y-t.height/2),s=Math.max(s,t.x+t.width/2),c=Math.max(c,t.y+t.height/2);let l=s-i,d=c-a,f=(i+s)/2,m=(a+c)/2,g=r.clientWidth,_=Math.min(g*.78,600),v=(g-32)/l,y=(_-32)/d,b=Math.min(v,y,1.5);u(Math.max(.25,b)),p(g/(2*Math.max(.25,b))-f),h(_/(2*Math.max(.25,b))-m)},[]),te=(0,y.useCallback)(()=>{if(I.length===0){u(1),p(0),h(0);return}let e=o.current;if(!e)return;let t=1/0,n=1/0,r=-1/0,i=-1/0;for(let e of I)t=Math.min(t,e.x-e.width/2),n=Math.min(n,e.y-e.height/2),r=Math.max(r,e.x+e.width/2),i=Math.max(i,e.y+e.height/2);let a=r-t,s=i-n,c=(t+r)/2,l=(n+i)/2,d=e.clientWidth,f=Math.min(d*.78,600),m=(d-32)/a,g=(f-32)/s,_=Math.min(m,g,1.5);u(Math.max(.25,_)),p(d/(2*Math.max(.25,_))-c),h(f/(2*Math.max(.25,_))-l)},[I]),ne=(0,y.useCallback)(()=>u(e=>Math.min(Ce,e+Z)),[]),ae=(0,y.useCallback)(()=>u(e=>Math.max(Se,e-Z)),[]),z=(0,y.useCallback)(e=>{e.preventDefault();let t=o.current.getBoundingClientRect(),n=e.clientX-t.left,r=e.clientY-t.top,i=-Math.sign(e.deltaY)*Z,a=Math.max(Se,Math.min(Ce,l+i)),s=a/l;p(e=>n-s*(n-e)),h(e=>r-s*(r-e)),u(a)},[l]),B=(0,y.useCallback)(e=>{e.button===0&&(_(!0),v.current={x:e.clientX,y:e.clientY,px:f,py:m})},[f,m]);(0,y.useEffect)(()=>{if(!g)return;let e=e=>{let t=(e.clientX-v.current.x)/l,n=(e.clientY-v.current.y)/l;p(v.current.px+t),h(v.current.py+n)},t=()=>_(!1);return window.addEventListener(`mousemove`,e),window.addEventListener(`mouseup`,t),()=>{window.removeEventListener(`mousemove`,e),window.removeEventListener(`mouseup`,t)}},[g,l]);let V=(0,y.useDeferredValue)(b),H=(0,y.useRef)(()=>{});(0,y.useEffect)(()=>{H.current=()=>{let e=s.current;e&&Te(e,N,P,I,A,V,E)}},[I,A,V,E,N,P]),(0,y.useEffect)(()=>{let e=requestAnimationFrame(()=>H.current());return()=>cancelAnimationFrame(e)},[I,V,E]);let U=(0,y.useCallback)((e,t)=>{let n=s.current;if(!n)return null;let r=n.getBoundingClientRect(),i=(e-r.left)/l,a=(t-r.top)/l;for(let e=I.length-1;e>=0;e--){let t=I[e];if(i>=t.x-t.width/2&&i<=t.x+t.width/2&&a>=t.y-t.height/2&&a<=t.y+t.height/2)return e}return null},[I,l,f,m]),W=(0,y.useCallback)(e=>{let t=U(e.clientX,e.clientY);if(t!==b&&x(t),t!==null){let t=o.current.getBoundingClientRect();C({x:e.clientX-t.left,y:e.clientY-t.top}),T(!0)}else T(!1)},[U,b]),G=(0,y.useCallback)(()=>{x(null),T(!1)},[]),K=(0,y.useCallback)(t=>{let n=Math.abs(t.clientX-v.current.x),r=Math.abs(t.clientY-v.current.y);if(n>3||r>3)return;let i=U(t.clientX,t.clientY);if(i!==null){let t=I[i],n=e.find(e=>e.id===t.id);n&&c(`/tags/${n.slug}`)}},[U,c,I,e]);if(e.length===0)return(0,Y.jsxs)(`div`,{className:`flex flex-col items-center justify-center py-16 text-center`,children:[(0,Y.jsx)(`span`,{className:`text-4xl text-muted-foreground/30 mb-4`,children:`🏷️`}),(0,Y.jsx)(`p`,{className:`text-lg font-medium text-foreground`,children:`暂无标签`}),(0,Y.jsx)(`p`,{className:`mt-2 text-sm text-muted-foreground`,children:`标签正在整理中`})]});let q=b===null?null:I[b],J=q?e.find(e=>e.id===q.id):null;return(0,Y.jsxs)(`div`,{ref:o,className:`w-full`,children:[(0,Y.jsxs)(`div`,{className:`flex items-center justify-between mb-4 flex-wrap gap-2`,children:[(0,Y.jsx)(`div`,{className:`flex items-center gap-1.5`,children:ie.map(e=>(0,Y.jsx)(`button`,{type:`button`,onClick:()=>a?.(e),title:re[e].label,className:[`inline-flex items-center justify-center w-9 h-9 rounded-lg text-lg transition-all duration-200 select-none`,e===t?`bg-primary text-primary-foreground shadow-sm scale-110`:`text-muted-foreground hover:text-foreground hover:bg-accent`].join(` `),children:re[e].icon},e))}),(0,Y.jsxs)(`div`,{className:`flex items-center gap-1`,children:[(0,Y.jsx)(`button`,{type:`button`,onClick:ae,title:`缩小`,className:`inline-flex items-center justify-center w-8 h-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors`,children:(0,Y.jsx)(i,{className:`h-4 w-4`})}),(0,Y.jsxs)(`span`,{className:`text-xs text-muted-foreground font-mono w-12 text-center select-none`,children:[Math.round(l*100),`%`]}),(0,Y.jsx)(`button`,{type:`button`,onClick:ne,title:`放大`,className:`inline-flex items-center justify-center w-8 h-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors`,children:(0,Y.jsx)(r,{className:`h-4 w-4`})}),(0,Y.jsx)(`button`,{type:`button`,onClick:te,title:`重置视图`,className:`inline-flex items-center justify-center w-8 h-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent transition-colors ml-1`,children:(0,Y.jsx)(n,{className:`h-3.5 w-3.5`})})]})]}),(0,Y.jsxs)(`div`,{className:`relative overflow-hidden rounded-lg border border-border bg-background select-none`,style:{width:`100%`,height:ee.h||500},onWheel:z,children:[(0,Y.jsx)(`div`,{className:`absolute top-0 left-0 origin-top-left`,style:{transform:`translate(${f*l}px, ${m*l}px) scale(${l})`,width:N,height:P},children:(0,Y.jsx)(`canvas`,{ref:s,className:`block cursor-grab active:cursor-grabbing`,style:{touchAction:`none`,width:N,height:P},onMouseMove:W,onMouseLeave:G,onMouseDown:B,onClick:K})}),w&&J&&(0,Y.jsxs)(`div`,{className:`absolute pointer-events-none px-2.5 py-1.5 rounded-md text-xs font-medium shadow-md border border-border bg-popover text-popover-foreground whitespace-nowrap transition-opacity duration-150 z-10`,style:{left:S.x+14,top:S.y-10,transform:`translateY(-100%)`},children:[J.name,(0,Y.jsxs)(`span`,{className:`ml-1.5 text-muted-foreground font-normal`,children:[`(`,J.articleCount,` 篇)`]})]})]}),I.length<e.length&&(0,Y.jsxs)(`p`,{className:`mt-2 text-center text-xs text-muted-foreground`,children:[`已显示 `,I.length,`/`,e.length,` 个标签 （部分标签因形状空间不足未显示）`]})]})}function $(e){let t=e.articleCount||0;if(e.children)for(let n of e.children)t+=$(n);return t}function De(e){return[...e].sort((e,t)=>{let n=$(e);return $(t)-n}).map(e=>({...e,children:e.children?De(e.children):void 0}))}var Oe=(0,y.memo)(function e({node:n,depth:r=0}){let[i,o]=(0,y.useState)(r<1),s=n.children&&n.children.length>0,c=(0,y.useMemo)(()=>$(n),[n]);return(0,Y.jsxs)(`div`,{children:[(0,Y.jsxs)(`div`,{className:`group flex items-center gap-2`,style:{paddingLeft:r*20},children:[s?(0,Y.jsx)(`button`,{onClick:()=>o(!i),className:`flex-shrink-0 h-6 w-6 flex items-center justify-center rounded text-muted-foreground hover:text-foreground hover:bg-accent transition-colors`,children:i?(0,Y.jsx)(t,{className:`h-4 w-4`}):(0,Y.jsx)(u,{className:`h-4 w-4`})}):(0,Y.jsx)(`span`,{className:`flex-shrink-0 w-6`}),(0,Y.jsxs)(l,{to:`/categories/${n.slug}`,className:`flex-1 flex items-center justify-between rounded-lg px-3 py-2.5 transition-all hover:bg-accent group-hover:text-primary`,children:[(0,Y.jsxs)(`div`,{className:`flex items-center gap-2 min-w-0`,children:[(0,Y.jsx)(a,{className:`h-4 w-4 text-primary flex-shrink-0`}),(0,Y.jsx)(`span`,{className:`font-medium text-foreground truncate`,children:n.name}),n.description&&(0,Y.jsxs)(`span`,{className:`text-xs text-muted-foreground truncate hidden sm:inline`,children:[`— `,n.description]})]}),(0,Y.jsxs)(`span`,{className:`flex-shrink-0 ml-2 font-mono text-xs text-muted-foreground`,children:[c,` 篇`]})]})]}),s&&i&&(0,Y.jsx)(`div`,{className:`mt-0.5`,children:n.children.map(t=>(0,Y.jsx)(e,{node:t,depth:r+1},t.id))})]})});function ke(){let[t,n]=(0,y.useState)(`categories`),[r,i]=(0,y.useState)(`circle`),{data:o,isLoading:s}=m(`/categories`,p,h),{data:c,isLoading:u}=m(`/tags`,p,h),{data:d,isLoading:f}=m(`/columns`,p,h),b=(Array.isArray(o)?o:o?.list)||[],x=(Array.isArray(c)?c:c?.list)||[],S=(Array.isArray(d)?d:d?.list)||[],C=(0,y.useMemo)(()=>De(b),[b]);return(0,Y.jsxs)(`div`,{className:`space-y-8`,children:[(0,Y.jsx)(g,{title:`发现`,description:`按分类、专栏或标签探索文章。`}),(0,Y.jsxs)(`div`,{children:[(0,Y.jsx)(`span`,{className:`font-mono text-xs font-semibold text-primary tracking-widest uppercase`,children:`探索内容`}),(0,Y.jsx)(`h1`,{className:`mt-4 text-3xl font-bold tracking-tight text-foreground md:text-4xl`,children:`发现`}),(0,Y.jsx)(`p`,{className:`mt-4 max-w-2xl text-muted-foreground leading-relaxed`,children:`按分类、专栏或标签探索文章，找到你感兴趣的内容。`})]}),(0,Y.jsxs)(`div`,{className:`flex items-center gap-2 flex-wrap`,children:[(0,Y.jsxs)(`button`,{onClick:()=>n(`categories`),className:`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${t===`categories`?`bg-primary text-primary-foreground`:`border border-border text-muted-foreground hover:bg-accent hover:text-foreground`}`,children:[(0,Y.jsx)(a,{className:`h-4 w-4`}),` 分类`]}),(0,Y.jsxs)(`button`,{onClick:()=>n(`columns`),className:`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${t===`columns`?`bg-primary text-primary-foreground`:`border border-border text-muted-foreground hover:bg-accent hover:text-foreground`}`,children:[(0,Y.jsx)(e,{className:`h-4 w-4`}),` 专栏`]}),(0,Y.jsxs)(`button`,{onClick:()=>n(`tags`),className:`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${t===`tags`?`bg-primary text-primary-foreground`:`border border-border text-muted-foreground hover:bg-accent hover:text-foreground`}`,children:[(0,Y.jsx)(v,{className:`h-4 w-4`}),` 标签`]})]}),t===`categories`?s?(0,Y.jsx)(`div`,{className:`space-y-3`,children:Array.from({length:6}).map((e,t)=>(0,Y.jsx)(`div`,{className:`animate-pulse rounded-lg border border-border bg-card p-4`,children:(0,Y.jsx)(`div`,{className:`h-5 w-32 rounded bg-muted`})},t))}):C.length===0?(0,Y.jsxs)(`div`,{className:`flex flex-col items-center justify-center py-16 text-center`,children:[(0,Y.jsx)(a,{className:`h-12 w-12 text-muted-foreground/30 mb-4`}),(0,Y.jsx)(`p`,{className:`text-lg font-medium text-foreground`,children:`暂无分类`}),(0,Y.jsx)(`p`,{className:`mt-2 text-sm text-muted-foreground`,children:`分类正在整理中`})]}):(0,Y.jsx)(_,{className:`rounded-xl border border-border bg-card p-4 md:p-6`,children:C.map(e=>(0,Y.jsx)(Oe,{node:e},e.id))}):t===`columns`?f?(0,Y.jsx)(`div`,{className:`grid grid-cols-1 sm:grid-cols-2 gap-4`,children:Array.from({length:4}).map((e,t)=>(0,Y.jsx)(`div`,{className:`animate-pulse rounded-xl border border-border bg-card p-5`,children:(0,Y.jsxs)(`div`,{className:`flex items-start gap-4`,children:[(0,Y.jsx)(`div`,{className:`h-10 w-10 shrink-0 rounded-lg bg-muted`}),(0,Y.jsxs)(`div`,{className:`min-w-0 flex-1 space-y-2`,children:[(0,Y.jsx)(`div`,{className:`h-5 w-24 rounded bg-muted`}),(0,Y.jsx)(`div`,{className:`h-4 w-full rounded bg-muted`})]})]})},t))}):S.length===0?(0,Y.jsxs)(`div`,{className:`flex flex-col items-center justify-center py-16 text-center`,children:[(0,Y.jsx)(e,{className:`h-12 w-12 text-muted-foreground/30 mb-4`}),(0,Y.jsx)(`p`,{className:`text-lg font-medium text-foreground`,children:`暂无专栏`}),(0,Y.jsx)(`p`,{className:`mt-2 text-sm text-muted-foreground`,children:`专栏正在整理中`})]}):(0,Y.jsx)(`div`,{className:`grid grid-cols-1 sm:grid-cols-2 gap-4`,children:S.map(t=>(0,Y.jsx)(l,{to:`/categories/${t.slug}`,className:`group rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/30 hover:shadow-sm`,children:(0,Y.jsxs)(`div`,{className:`flex items-start gap-4`,children:[(0,Y.jsx)(`div`,{className:`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary`,children:(0,Y.jsx)(e,{className:`h-5 w-5`})}),(0,Y.jsxs)(`div`,{className:`min-w-0 flex-1`,children:[(0,Y.jsx)(`h3`,{className:`font-semibold text-foreground group-hover:text-primary transition-colors`,children:t.name}),(0,Y.jsx)(`p`,{className:`mt-1 text-sm text-muted-foreground leading-relaxed`,children:t.description})]})]})},t.id))}):u?(0,Y.jsx)(`div`,{className:`flex flex-wrap gap-3`,children:Array.from({length:12}).map((e,t)=>(0,Y.jsx)(`div`,{className:`animate-pulse rounded-lg bg-muted px-4 py-2`,children:(0,Y.jsx)(`div`,{className:`h-4 w-16 rounded bg-muted-foreground/10`})},t))}):x.length===0?(0,Y.jsxs)(`div`,{className:`flex flex-col items-center justify-center py-16 text-center`,children:[(0,Y.jsx)(v,{className:`h-12 w-12 text-muted-foreground/30 mb-4`}),(0,Y.jsx)(`p`,{className:`text-lg font-medium text-foreground`,children:`暂无标签`}),(0,Y.jsx)(`p`,{className:`mt-2 text-sm text-muted-foreground`,children:`标签正在整理中`})]}):(0,Y.jsx)(`div`,{className:`rounded-xl border border-border bg-card p-4 md:p-6`,children:(0,Y.jsx)(Ee,{tags:x,shape:r,onShapeChange:i})})]})}export{ke as default};